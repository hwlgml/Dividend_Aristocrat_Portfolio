...
class OptionTrading:
    ...
    def iv_mean_reversion(self):
        ...
        trapezoid_z = max(clip_z - self.min_z, 0) if self.iv_zscore > 0 else min(clip_z + self.min_z, 0)
        # target_position = int(-self.option_limit * clip_z / self.max_z)
        target_position = int(-self.option_limit * trapezoid_z / (self.max_z - self.min_z))
